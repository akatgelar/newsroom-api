...
class VideoController extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        $count_slug = Video::where([['title', '=', $data['title']]])->count('id');
        if($count_slug > 0) {
            $count_slug += 1;
            $data['slug'] .= '-'. (string)$count_slug;
        }

        $query = Video::create($data);
        $query = json_decode(json_encode($query), true);
        ksort($query);
        ...
        app()->call('App\Http\Controllers\LogController@InsertLog', [$token, 'INSERT', 'Video', $result['data']['id']]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $header = $request->header('Authorization');
        $token = substr($header, 7);
        app()->call('App\Http\Controllers\LogController@InsertLog', [$token, 'UPDATE', 'Video', $result['data']['id']]);
    ...
    public function delete($id, Request $request)
    {
        ...
        $header = $request->header('Authorization');
        $token = substr($header, 7);
        app()->call('App\Http\Controllers\LogController@InsertLog', [$token, 'DELETE', 'Video', $id]);
